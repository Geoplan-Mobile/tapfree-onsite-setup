...
> ℹ️ **gpi-tapfree 2.0.3 이상 사용 시 — `offsetNs` 값은 임의값으로 작성**  
> 2.0.3 부터 라이브러리가 캘리브레이션 offset 을 사용하지 않으므로, `anchorInfo.offsets[].offsetNs` 의 **실측값은 필요 없다**.  
> 다만 Edge 와의 통신 프로토콜이 변경된 것은 아니므로 **항목 자체는 빈 값으로 두지 말고 `0.0` 등 임의값으로 반드시 작성** 해야 한다 (앵커별 항목 / `mac` 필드는 정상 작성, `offsetNs` 만 임의).  
> 이 경우 [06. iPhone 캘리브레이션](./06-iphone-calibration.md) 단계 자체를 건너뛸 수 있다.
...
- [ ] **UWB 앵커에 세팅된 Session ID** — 앵커 수령 시 안내받은 값 (**tapfree 용은 별도 안내 없다면 `5010`**)
